...
test("empty and user-only SDK sessions survive eviction with identity, task receipts and appendable history", async () => {
  ...
  assert.deepEqual(restored.submitInput("hello", [], "command"), { ...accepted, duplicate: true });
...
test("active work, operations, unsaved model choices, recent use and failed persistence prevent eviction", async () => {
  ...
  const model = await registry.select(null, true); model._currentModelRef = { providerId: "missing", modelId: "new" };
