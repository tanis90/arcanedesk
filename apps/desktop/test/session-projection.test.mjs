...
test("snapshot restores cumulative text, thinking and a tool's original start time", () => {
  ...
  const started = projection.publish({ type: "tool_start", toolCallId: "t", toolName: "bash", args: { command: "job" } });
  now = 100;
  ...
  assert.equal(started.startedAt, snapshot.tools[0].startedAt, "live event and recovery share one clock");
  ...
  const ended = projection.publish({ type: "tool_end", toolCallId: "t", toolName: "bash", isError: true });
  assert.equal(ended.startedAt, 10);
  assert.equal(ended.finishedAt, 100);
  assert.equal(projection.snapshot().tools[0].finishedAt, ended.finishedAt);
