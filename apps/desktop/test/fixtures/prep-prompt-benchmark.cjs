...
module.exports = async function benchmark({ evaluate, report, save, root, runId, store, page, origin, revision, samples, setHost, resumePath }) {
  ...
  const cases=["create_npc","grant_items","edit_image","scene_layout","conditions"];
  ...
  const verify=async(id,f)=>evaluate(`(()=>{
  ...
  for(let sample=0;sample<samples;sample++)for(const caseId of cases)for(const arm of sample%2?["tools","js"]:["js","tools"]){
    ...
    // and replace the product routing text equally in both experiment arms.
    assert.ok(host.session.systemPrompt.includes(productionPrep),"Prep preamble anchor changed");
    const prompt=host.session.systemPrompt.replace(productionPrep,common+(arm==="tools"?" 优先使用结构化工具；未覆盖的操作才使用 browser_evaluate。":" 使用 browser_evaluate 编写 JavaScript 调用原生 Document API 完成世界操作。"));
