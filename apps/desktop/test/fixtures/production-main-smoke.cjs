...
const server = http.createServer(async (req, res) => {
  ...
  if (retryScenario && ["A", "C"].includes(tag) && requests.filter(item => item === tag).length === 1) {
    res.writeHead(400, { "content-type": "application/json" });
    res.end(JSON.stringify({ error: { message: "overloaded: controlled retry fixture", type: "overloaded_error" } }));
    return;
  }
...
app.on("will-quit", () => {
  try {
    ...
    if (retryScenario) {
      ...
      assert.equal(requests.filter(tag => tag === "C").length, 1);
      assert.equal(hostB.tasks.task.state, "completed");
      console.log("PASS production retry: real SDK backoff, cross-mode/reload stage recovery and isolated cancellation");
      return;
    }
    if (contextIsolation) {
...
(async () => {
  ...
  if (retryScenario) {
    ...
    await until(() => a.projection.snapshot().retry?.attempt === 1, "SDK actually schedules retry");
    const taskId = a.tasks.task.id;
    await ui('busy && !document.getElementById("conversation-retry").hidden');
    ...
    assert.equal(await evaluate('document.getElementById("conversation-retry").hidden'), true);
    ...
    await ui('busy && !document.getElementById("conversation-retry").hidden');
    assert.equal(a.tasks.task.id, taskId);
    await until(() => requests.filter(tag => tag === "A").length === 2, "SDK retries the same task");
    await ui('messages.textContent.includes("A partial")');
    assert.equal(await evaluate('document.getElementById("conversation-retry").hidden'), false, "retry stage survives SDK agent_start");
    const retryReloaded = new Promise(resolve => window.webContents.once("did-finish-load", resolve));
    window.reload(); await retryReloaded;
    await ui('busy && messages.textContent.includes("A partial") && !document.getElementById("conversation-retry").hidden');
    streams.get("A").finish();
    await ui('!busy && document.getElementById("conversation-retry").hidden && messages.textContent.includes("A final result")');
    assert.equal(a.tasks.task.state, "completed");
    assert.equal(a.tasks.task.id, taskId);
    await evaluate('input.value = "production-C"; submit()');
    await ui('busy && !document.getElementById("conversation-retry").hidden');
    await evaluate('stop.click()');
    await openHost(hostB);
    await until(() => a.tasks.task.state === "stopped", "retry cancellation settles A");
    await sleep(8500);
    assert.equal(requests.filter(tag => tag === "C").length, 1, "cancelled backoff never starts another model request");
