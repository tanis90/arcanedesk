module.exports = async function queueChecks({ evaluate, sourceId, targetId, report, save, root, runId, revision, page }) {
  ...
  const scheduler = new revision.ExecutionScheduler({ capacity: 1 });
  ...
  const service = new FoundryServices({sessionId:runId,directory:path.join(root,runId,"queue-operations"),mode:"combat",
    withPage:(signal,operation)=>scheduler.acquire({sessionId:runId,taskId:"queued-write"},signal,()=>{queuedAt=performance.now();waiting();}).then(lease=>{admittedAt=performance.now();return operation().finally(()=>lease.release());}),
  ...
  for(let i=0;i<10;i++) {
    ...
    const blocker=await scheduler.acquire({sessionId:runId,taskId:"fixture-hold"});
