...
Wizard: use 6 + 2 per additional wizard level as the default total spellbook size (14 at level 5), with level-appropriate cantrips, spell levels and prepared count; required Fireball is prepared. This NPC task does not require reconstructing the exact level-by-level acquisition history or limiting how many of those spells share the highest allowed level. Cleric: level-appropriate cantrips, ordinary prepared list, and domain spells always prepared in addition. Do not add every spell in a choice pool. Do not give higher-level spells or slot overrides to match a guess.
