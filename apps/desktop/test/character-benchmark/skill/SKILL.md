...
For a new character: standard array 15/14/13/12/10/8 plus race and ordinary ASI, no feats. Human is nonvariant; dwarf is hill dwarf; unspecified wizard subclass is evocation. Explicit ability values are final. Choose legal class skills/languages and required choices, without asking DM. Full health/resources at completion. Use one legal starting equipment combination, prioritizing the requested weapon: resolve equipment the DM named exactly, and take everything else from common knowledge in one batch without item-by-item research. No background, portrait search or Tokens.
...
New Hit Dice use monster size, never the class die: tiny d4, small d6, medium d8, large d10, huge d12, gargantuan d20. Each added class level grants (size die / 2 + 1) + final CON modifier HP — the native dnd5e per-die average, rounded up (d8 → 5 + CON), minimum 1 per level. HP = source maximum HP + added levels × that per-level value. No maximum-first-class-die bonus, and the source HP baseline is never recomputed when CON changes.
