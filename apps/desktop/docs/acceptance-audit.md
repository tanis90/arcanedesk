...
结论：整体未完成。G1–G6、G9 的指定修复及联合场景已完成；G7 的 Windows 关闭/取消/最小化/停止退出和多尺寸渲染布局已有证据，系统托盘及通知真实点击尚未通过。M8n3 性能复核通过。G8 最终复核仍在进行。

下表以当前证据为准，历史修复过程见 [milestones](milestones.md)。G6 关闭仅指其列出的生产联合场景，不代表所有规格条款都已完成：单工具失败后继续、慢停止操作说明、停止保留实际工具产物、生产环境停止并删除运行会话，原表中的这些待集成项仍保留，汇总为 G10，不能被 G6 或 G5 的修复记录掩盖。
...
| 14 | 超额度取消排队 | E9 自动化通过 |
| 15 | 删除执行中会话 | E7/E11 分层通过；真实主进程运行中停止并删除交互待补（G10）；G5 仅关闭替代启动失败恢复 |
...
| 正确性不能用最终刷新替代 | E6/E7/E8 为独立断言；G1–G5 已修复；G7/G10 未完成前整体仍不通过 |
...
| G10 剩余行为集成 | 保留原条款 §5 补充-1、§8.1-3/4、§8.2-2 与 §11.1-15 的证据缺口 | 实际工具失败后任务继续；慢停止期间显示正在等待的操作；停止后已有真实产物保留；生产主进程等待停止后才删除运行会话 |
