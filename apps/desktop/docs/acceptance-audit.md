...
M8m5 更新：G6 实际 Foundry 联合场景由 E18 通过，见 [实际资源验收](foundry-resource-acceptance.md)。隔离 test001 副本中，A 的真实 browser_evaluate 持有页面资源，B 等待后被停止且未执行；A 创建唯一 JournalEntry 并经 SDK 返回终稿，测试文档已清理。G6 已关闭；G7 及最终性能/规格复核仍待完成。
