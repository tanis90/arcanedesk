...
配置预检补充：首条真实 AgentHost 报告 benchmark-1788870237146 在 0.73 秒、零工具调用时 HTTP 400，供应商不支持 developer 角色。思考能力标记让 Pi 改用了该角色，原小探针没有系统提示词，未覆盖此边界。失败保留为配置失败，不计搜索效果。已补 supportsDeveloperRole=false，并以带系统提示词的真实探针确认 system 角色及 off/on 参数通过；18 项测试覆盖实际 Pi 出站角色。两臂仍共享同一修正后的 ProviderStore，重新开始完整三对新环境块，旧失败不覆盖。

完整六次报告 benchmark-1788870358459 已结束，逐次原始索引、参数、错误及数据复核见[显式批量实验结果](prep-search-explicit-batch-results.json)。对照 67.22／67.15／165.37 秒，11／11／19 调用，正确 1/3；候选 62.30／88.45／33.56 秒，8／13／5 调用，正确 0/3。120 秒内正确率同为 1/3 对 0/3；无超时、无供应商错误。所有请求发送 thinking=false，reasoning tokens=0，skill hash 一致。

三个候选均采用三次单项查询，真正批量调用为零；对照三个样本均出现数组字符串查询。显式参数没有显示正确率收益。候选失败分别是重复火球术、缺少人类身份、法术位与火球术；末个对照重建了同名角色并残留高环资源。当前问题主要是任务正确性，不是硬超时过短，也不能把错误任务 33 秒作为收益。

决定停止批量参数路线：撤回 string/array 与 queries 两种实验合同及 SDK 批量实现，恢复 ddf1132 的单查询＋identifier 匹配，原记录和提交保留。停止继续在法师上补提示；当前示例指南冻结，仅作实验，尚不推广到产品。维护代价：两轮增加的合同、分页附加字段和测试分支已撤回；保留已证实的搜索漏匹配修复与 Qwen 兼容修复。

下一阶段采用冻结后的单查询代码和同一指南，牧师题比较当前产品 18 工具与实验指南 16 工具（native-skill），三对交错、180 秒、Qwen off，检验工作流程能否迁移；这不是单因素搜索实验，不混算收益。随后六类原有备团题每题一对做功能/轨迹回归，不以一次回归宣称稳定速度。牧师验收器已在本地真实临时内存 Actor 上通过 8 项检查、无世界写入；模型尚未做此题。无论迁移结果如何，本轮停止新增特例，进入最终取舍审计。

### 14.28 冻结方案的牧师迁移验证

代码冻结 469b4b9、相同单查询工具实现；产品 18 工具与实验指南 16 工具各三次交错，报告 benchmark-1788871018914。模型、思考 off、180 秒配置保持一致，迁移过程中不更改工具、skill 或验收器。

运行中发现验收器边界：effects 比较只排除 _id，误把原生导入时更新的 _stats 版本/时间元数据当成机制改变。首条 skill 样本还存在真正的 2024 版 Bless 来源及额外 Spell Changes 效果；第二条是 2014 来源，效果差异仅 _stats。本地实际 Activity 的 duration.concentration 与 requiresConcentration 均为 true，原始 toObject 的 false 是继承前存储值，不是缺专注。最终须保留原始评分，额外列出排除无关导入元数据并检查规则版本后的独立复核，不把评测修正算作模型改善。当前块不重跑、不换验收器。

六次均已结算，见[牧师迁移结果](prep-priest-transfer-results.json)。产品三次为 92.71／103.86／68.53 秒、23／26／15 调用；指南三次为 75.58／47.38／88.70 秒、11／8／15 调用。原始验收两边均 0/3；保留原始报告后独立只读复核，产品 0/3、指南 1/3（47.38 秒）。所有请求 thinking=false，未发生超时。产品三次施法等级/法术位均失败，指南前两次这两项通过；完整任务还存在错版资源、额外效果、重复对象或残留资源问题，不能据此推广。

验收器后续版本改为 prep-npc-priest-transfer-draft2：机制比较忽略 _id 与 _stats，增加明确 2014 法术来源检查；5 项测试覆盖元数据更新可接受、效果数值变化不可接受、错版不可接受。修正后结果单独标为 posthoc，不回写原始 success，也不补跑本题刷分。牧师从此为已见题。本轮决定不推广实验 skill、不移除产品 actor_create/update；保留实验轨迹与字段知识，停止新增题目特例，转入六类原有题回归和最终结算。

### 14.29 修正后 loop 的回归与最终结算

冻结 580b387 的六类原有题各一对完成，报告 benchmark-1788871657166，见[回归逐次记录](prep-corrected-baseline-regression-results.json)。工具 6/6、裸 JS 3/6；通过的任务均在 120 秒内。每题只有一对，只支持本地功能/轨迹结论，不支持稳定延迟或泛化成功率。

| 用例 | 裸 JS 秒／验收 | 工具 秒／验收 |
| --- | --- | --- |
| 克隆角色 | 19.42／通过 | 29.69／通过 |
| 授予物品 | 38.50／通过 | 20.69／通过 |
| 字段与图片同步 | 10.49／失败 | 15.75／通过 |
| 非当前场景布局 | 25.76／通过 | 19.62／通过 |
| 状态 | 14.64／失败 | 11.06／通过 |
| 本地图片上传 | 17.84／失败 | 12.02／通过 |

JS 失败具体为 AC 与存量 Token 图片未修改、实际状态为空、将 Windows 本地路径直接写入图片字段而未上传。工具克隆最终通过但仍有 4 次参数错误，不能认为 actor_create 已流畅。六题自有 fixture 已按报告完成清理；迁移和开发题保留供复核，未清理用户原有对象。所有请求均 thinking=false、reasoning tokens=0。

最终取舍：

| 项目 | 决定与依据 |
| --- | --- |
| Item identifier 搜索 | 保留；真实中英文漏匹配已修复，单查询合同不扩张。没有稳定提速承诺。 |
| 批量 query/queries | 已撤回；两轮未证明模型实际批量使用或正确率收益，停止新增格式特例。 |
| Qwen 接入 | 保留已验证型号/官方地址的思考参数和 system 角色兼容；每请求记录避免环境混比。 |
| NPC 实验 skill | 不推广；字段指导有帮助，但法师和未见牧师均未稳定完成。保留 fixture、失败轨迹及研究知识。 |
| 普通 actor_create/update | 产品仍保留，未因实验而移除。创建参数错误是已知问题，当前没有足够可靠的替代流程。 |
| 图片、状态、授予物品、场景工具 | 保留；回归支持其正确性价值，尤其是路径上传与存量 Token 同步，不能随普通字段工具一起删去。 |
| auto pack／休息 | 未修改、未新增；继续按既有 TODO 纪律处理。 |

时间结论：校正后的 24 条正式任务中，没有正确完成且超过 120 秒的样本；165 秒的法师是失败任务。仍保留 180 秒实验硬超时来观察长尾，不以这些数据承诺“再等一分钟就会成功”。主要未解问题是选源、原生字段、重复创建和资源配置的正确性。不同思考配置的旧数据不合并计算工具收益。
...
## 16. 创建 NPC：生产查包指南注入实验（2026-09-09，待实跑）

复核 165 秒失败轨迹后发现，原 native NPC benchmark 只加载实验指南，没有加载生产 arcane-actor-update；后者已经提供 Arcane 2014 包位置、优先级和双语检索方法。假设：暴露该指南并要求查素材前读取，可减少包发现和错误匹配的往返。它不保证解决规则理解错误或写入后错误恢复。

已新增独立 pack-skill 比较入口，两臂保持工具、原生 NPC 指南、题目与验收相同，候选额外注入未修改的生产指南地址，记录来源及 hash。全文按需读取，不预塞答案或条目 UUID。生产指南与 auto pack 均未修改。具体命令和测量边界见[benchmark 使用文档](prep-benchmark.md#生产查包-skill-地址注入对照)。当前只完成语法和 diff 检查，尚无新的模型实验结果，不宣称提速或正确率改善。

## 17. 规则知识来源与组织调研（2026-09-09，尚未实现）

用户指出不能在未检查原始资料前假设400页PDF能按目录直接变成skill。已检索并读取5thSRD、5e-database、Tabyltop CC-SRD、srd-builder四个固定提交；找到现成文本和结构化资料，因此未启动MinerU全书转换。5thSRD是当前文本候选，5e-database用于有限交叉校验；7个职业1100个法术位单元格一致，不代表全库官方准确性认证。未加载新的规则skill、未增加生产工具、未运行新模型实验、未改auto pack。

组织提案、来源链接、实测结构、局限和后续验证方式见[SRD知识来源审查与组织提案](srd-knowledge-design.md)，原始统计见[审查记录](srd-knowledge-source-audit.json)。重点是保留原文与表格出处，区分Wizard职业与Mage数据块，用按需批量查询取得规则证据；不再用禁止质疑系统推导的行为约束代替独立判断，也不将玩家职业完整升级流程强加到NPC任务。采用资料与检索方案仍需验证，本文不是已交付的知识库。

## 18. SRD规则索引skill落地（2026-09-09）

按用户最终决定，交付“原文Markdown + LLM索引”，取消第17节提案中提前建设专用查询脚本/结构化查询单元的安排。新增[arcane-dnd5e-rules/SKILL.md](../skills/prep/arcane-dnd5e-rules/SKILL.md)，既提供规则导航也说明如何使用现有PowerShell/read查阅。954份正文随prep bundle分发，附固定来源与署名；入口55个本地链接，不将全部正文预塞模型上下文。详情及差异见[调研文档第7节](srd-knowledge-design.md#7-实际交付索引skill不封装查询器)。

bundle revision 8→9，包目录校验名单同步更新；新增测试覆盖原文完整性、本地链接、打包解包迁移与真实Pi只发现一个skill。此交付没有增加Foundry工具，没有修改auto pack，没有启动新的模型benchmark。仅落地到源码与分发管线，尚未发布OSS bundle；模型正确率与延迟收益不作推断。

验证完成：Desktop source boundary通过，Desktop全套464项测试通过；新增两项覆盖954份原文与链接完整性、tar迁移后真实Pi只发现一个skill。首次测试夹具误把单skill目录传入要求技能集合根目录的publisher收集函数，修正测试调用后通过；生产收集逻辑未修改。尚无新模型效果数据。

## 19. 规则skill注入与thinking开关小测（2026-09-09）

用户明确备团优先正确完成、少返工，希望thinking开启，并允许两边测试。固定本地COS、Aliyun qwen3.7-plus、相同17工具和三份指南（实验NPC、生产查包、生产SRD规则含正文），各跑一次法师。关闭先跑，开启后跑；180秒实验上限，120秒体验目标。真实出站分别enable_thinking=false/true，reasoning_effort均省略，客户端high只是Qwen思考开关，不宣称存在独立high档。

| thinking | 时间 | 工具调用 / 模型工具轮次 | 规则读取 | 独立数据验收 |
| --- | --- | --- | --- | --- |
| off | 97.00秒 | 12 / 11 | 规则索引、Wizard、Human、Fireball；未读已暴露的查包skill | 失败：两份NPC。法术位等其余8项通过 |
| on | 134.67秒 | 13 / 8 | 三份指南及Wizard、Fireball、Weapons，合并读取 | 9项全通过；一份NPC，无工具错误 |

关闭组在创建后使用不存在的Actor.fromUuid验收，随后整段重放Actor.createDocuments，产生重复角色。开启组无此错误，合理合并读取；另有AC检查和修正，不能描述成完全没有任何修正。开启组超过120秒，但在180秒内正确完成。两边都取得正确的4/3/2规则依据。本轮不测试法术实际执行，不证明全角色机制完整，不改变原验收。

结果及轨迹摘要见[小测记录](prep-rules-thinking-pilot-results.json)，两个原始报告名1788929377072、1788929501202，保留成功与失败NPC供复核。三份skill hash、工具集合、工具/runtime hash相同；原始prompt hash因任务路径不同而不同。首版normalized hash漏掉Pi的正斜杠cwd表示，不能据此证明内容相等；已修复未来报告normalization v2，不重跑/改写本轮原始数据。

决策：后续备团主线实验采用thinking开启，以正确完成优先，off保留为消融对照。单对、固定顺序不能证明稳定胜率或速度；两边都注入规则skill，因此不能归因出规则skill单独收益。尚未修改App用户默认设置或启动新的自动迭代goal。

## 20. 规则skill有无对照（2026-09-09）

已完成三对法师NPC任务，thinking开启，固定相同17工具、NPC指南和生产查包指南，仅候选增加SRD规则skill与读取路由。顺序无/有、有/无、无/有；180秒硬上限、120秒体验目标。两次超时先停止和复核，后续仅继续未执行样本，没有重跑失败。

| 指标 | 无规则skill | 有规则skill |
| --- | --- | --- |
| 180秒内完成且数据通过 | 3/3 | 1/3，另2次超时 |
| 观察到的9项数据全通过 | 3/3 | 3/3，其中2次是停止后复核 |
| 120秒内正确完成 | 1/3 | 0/3 |
| 耗时 | 117.60、154.05、151.54秒 | 超时180.01、超时180.02、135.16秒 |
| 平均调用次数 | 10.33 | 17 |

结论：本轮不能证明规则skill改善正确率，当前注入方式限时完成表现更差，但第三对候选快16.38秒，不能推广为总会变慢。候选均实际读取规则，资料可达性成立；无规则对照也全部获得正确法术位，因此本题未提供规则纠错的正面证据。9项数据检查不等于整张卡规则完整认证。

耗时轨迹包含素材反复搜索、Collection API误用、HP/AC修正和额外界面检查，不能都归于thinking或规则阅读。全部工具执行累计每次0.36–1.14秒；工具之间的时间包含供应商延迟与输出生成。固定工具/runtime/指南hash、同臂normalized prompt hash和实际thinking开关已核对。

完整协议、逐次结果、超时处理和后续建议见[规则对照记录](prep-rules-ablation-results.md)，机器可读证据见[结果JSON](prep-rules-ablation-results.json)。当前保留规则查询能力，不宣称其已优化NPC创建；如验证知识收益，下一轮先定义存在规则分歧的任务和独立验收。未修改生产skill、auto pack，未启动额外迭代goal。

## 21. DeepSeek灰度模型补充小测（2026-09-09）

用户指定 `deepseek-v4.1-flash-expires-on-0910` 官方接口旁路试跑，Qwen3.7-plus继续主基线。独立加密测试profile，一对无规则→有规则，题目、17工具、指南、归一化提示词、runtime及9项验收与第20节一致。实际thinking开启且reasoning_effort=high；两家high含义不同，不作相等思考预算声明。

无规则53.08秒、17调用/14轮，有规则121.61秒、30调用/23轮，均9项通过、180秒内完成；120秒目标仅无规则条达成。有规则条有两次JS字段读取错误后恢复，未重复创建。该条读取前一条保留NPC的图片字段用于素材定位，世界不是隔离快照，此限制已明确记入结果。

本次速度相较Qwen历史样本有潜力，但供应商、时间与样本量不同，不更换基线、不宣称稳定优势。规则skill本对未带来验收收益而增加耗时。完整轨迹结论和配置见[DeepSeek灰度记录](prep-deepseek-gray-pilot-results.md)及[结果JSON](prep-deepseek-gray-pilot-results.json)。仅补充基准payload.thinking观测，未改生产接入逻辑、skill、auto pack或App默认模型。

补充人工审查：DeepSeek无规则条并非全对。人类步行速度未配置、CR5缺少独立评估依据，最终回复还错误声称法术没有activities（快照实际存在）。53秒只代表既定九项通过，不是整卡正确认证；原评分不追改，详见上述灰度记录的补充审查。

## 22. 转向系统成长数据的薄查询（2026-09-09）

用户纠正目标：未指定项应走默认，不应让模型自行设计完整角色。当前主线转为直接读取dnd5e职业/种族advancement，不依赖Actor Studio；规则正文注入假设停止继续优化，历史资料保留。实验薄工具只做读取、按目标等级展开和引用解析，不引入新的角色写入schema或升级执行器。

已写实验查询与两组共用skill，真实COS只读数据检查通过；准备Qwen和DeepSeek各一对“同skill裸JS vs额外查询工具”，180秒上限。新默认措辞两边一致，不与旧轮自由补充题目直接混算。设计、边界、预注册顺序与迭代结果见[车卡查询实验](prep-build-query-results.md)。尚未升级生产工具、未修改auto pack。

首轮现已完成：Qwen从160.19秒/12调用降到136.40秒/9调用，模型工具轮次10→6；DeepSeek从96.26秒/26调用降到28.89秒/8调用，轮次22→5。四次既定9项均通过；两个候选各一次查询，DeepSeek裸JS出现三次API错误而工具组没有。夹具白名单接线错误均发生在候选模型请求前，修复测试接线后继续未执行任务，不计为模型失败。

新假设有初步收益，保留为当前迭代主线。仍有重要局限：Qwen工具组漏用已返回的30尺移动；DeepSeek两臂落实移动。CR按下述用户修订不参与评价。不是整体全对，也不是单对即可确定稳定提速。下一步重点是来源默认/自动获得/待选择信息的清晰表达与采用，再做重复和迁移验证，暂不推广生产工具或扩展写入引擎。具体证据与限制见[结果JSON](prep-build-query-results.json)和实验记录。

## 23. 车卡任务评估口径修订（2026-09-09）

按用户决定，职业车卡任务不评估CR数值或合理性；此前第21/22节及历史人工审查中将CR作为质量缺陷的判断不再适用。历史原始数据保留，既有9项检查无CR评分，无需修改通过数。仍核对用户要求的有效属性/攻击/豁免等结果；若NPC底层由CR派生熟练，属于实现适配问题，不转化为用户CR要求。

规则验收依据任务语义：从头车卡核对完整的目标等级累计成长；复制狼人并追加五级冠军勇士战士，则核对原狼人能力保留、战士1–5级与冠军勇士到5级的累计增量，以及预先定义的HP/熟练/攻击动作合并规则。PC/NPC字段和执行路径可不同，不因NPC类型自动削减明确要求的职业能力。后续试验先固定这些默认与合并策略，再执行，不追改当前四次的评分。

## 24. 六题benchmark文档对齐稿（2026-09-09）

按用户要求只编写[六题Prompt与成功条件](prep-character-benchmark-cases-v1.md)，未运行模型、未修改工具或auto pack。纯车卡与追加职业各3题，逐题给出可直接提交的prompt、硬验收与不计分范围；CR排除。默认子职/种族分支、完整施法配置范围以及追加HP/熟练合并均明确标成提案，尚未确认，不当作已冻结标准。用户对齐后再准备来源快照、补齐查询与验收器，不先启动测试。

## 25. 六题实卡与验收器审阅轮（2026-09-09）

承接§24，本轮先验证benchmark自身，不启动模型对照。用arcane-cli连接本地COS受控Chrome，创建A1—B3六张审阅NPC卡，保留在“Benchmark 审阅 · 职业成长 v1”文件夹，供DM检查。没有修改auto pack。

实现位于[test/character-benchmark](../test/character-benchmark/README.md)：确定性样例创建、只读原始/派生数据采样、独立断言、错误快照测试与结果导出。六张卡已编码检查通过；6正例及22反例通过。源对象在复核时重新读回，比对原快照。详见[审卡记录](prep-character-review-v1.md)及[逐项结果](prep-character-review-v1.json)。

此轮结论仅是“这些检查能写入、读回并抓住对应错误”。不是六题规则覆盖已完整，不是模型成绩，也未冻结重要性。发现NPC原生可持有职业Item并派生等级/部分资源，但追加题仍有HP公式、混合生命骰等一致性问题待审；原物品保留不等于职业自动化已经可用。

下一步由DM根据实卡指出影响使用的缺口，确定核心/次要/排除项；随后调整合法默认、补齐验收覆盖，并去掉固定技能/装备选择等对模型等价解的错误限制，再接入计时对照。当前不把全部字段全绿当成最终成功，不按模型结果倒改门槛。CR不评分，auto pack改造纪律继续有效。


## 26. 追加职业：生命骰原则修正（2026-09-09）

用户确认：默认保留怪物原属性，只应用职业正常属性提升，除非DM指定；每追加一级使用怪物体型对应的一颗生命骰，不使用职业生命骰。依据2014 DMG p.283 Monsters with Classes。两组共用的[成长查询实验skill](../test/fixtures/prep-build-query-skill/SKILL.md)已落地该原则，工具返回的职业生命骰不能用于怪物追加题。尚未发布到生产skill bundle。

§25六张样例和结果为历史记录。原创建器/验收器按职业固定HP和较高熟练实现，不能直接运行作为新口径成绩；下一轮前必须修正、另建版本并重新验证。平均/取整与CON追溯计算仍需冻结。DMG熟练依据CR，与当前FVTT原生NPC class Item派生行为不同；产品口径讨论中，禁止改CR凑熟练值。CR自动评估仍未纳入benchmark。此轮只修改规则说明，不改世界角色或auto pack。


### 26.1 CR边界已确认

用户确认：追加职业不自动调整CR，明确标注“追加职业后的CR重评未实现”。保留来源CR；只有DM明确给出替代值时才按要求修改。不调用模型估算，不以职业等级固定换算CR，也不为迁就熟练加值而修改CR。狼人加五级战士与龙加五级施法者对整体威胁的影响不同，不能用统一增量替代评估。

CR准确性继续不参与benchmark评分；“按要求保留CR”是操作边界，并不代表该旧值已准确反映新怪物的遭遇难度。交付时简短说明未重评。此决定没有自动批准熟练加值的具体实现；熟练与FVTT派生的差异仍需在新轮前明确，不能把“保留CR”当作旧实现已正确。

共用实验skill及六题文档已同步；后续可能的CR能力仅列入[todo](todo.md#追加职业后的cr重评未实现)，不是本轮任务。


### 26.2 熟练沿用系统默认（用户确认）

沿用本地dnd5e5.3.3的NPC原生派生：以max(CR,职业等级)确定熟练加值。不为贴合DMG的CR-only规则要求agent添加ActiveEffect覆盖或手工修正，也不改CR凑数。这是明确接受的系统行为，不再作为缺陷。覆盖§26与§26.1中熟练口径待定的状态。

共用skill、benchmark policy及验收器同步；新增低CR高职业等级、高CR低职业等级和低等级边界测试。旧样例的熟练口径可沿用，HP仍需按体型生命骰规则更新。世界角色尚未添加过上述覆盖效果，无需清理。

## 27. 六题benchmark实现、试跑与验收修订完成

本轮目标已落实为可复用代码、真实模型试跑和[使用手册](prep-character-benchmark-manual.md)。六题的[任务合同](prep-character-benchmark-cases-v1.md)区分纯车卡与怪物追加职业，原生class/subclass为当前支持路径；未覆盖的等价实现转人工审查，不以字段形状代替规则正确性。

**首轮证据**：本地COS、Foundry13.351/dnd5e5.3.3，Qwen3.7-plus与DeepSeek灰度均thinking high。6题×2模型×2臂共24次原始尝试，均终止并只读审计；含4次超时和1次供应商错误，没有重跑到成功。A1四次180秒，之后按用户要求对新试次放宽为300秒，体验目标120秒不变。共同skill/查询工具指纹一致，17个共同工具与候选额外1个foundry_build_query的差异已核对。详见[结果表](prep-character-pilot-results.md)、[逐项数据](prep-character-pilot-regrade-v3.json)、[完整性审计](prep-character-pilot-integrity.json)。

**验收修订**：原v2误判了auto pack空/带前缀identifier、NPC种族表示、复制后嵌入ID与gear迁移；v3用审核后的来源UUID与实际配置识别。另补上越级能力、已收录法术职业/环阶、武器活动、法术攻击/DC和技能关联属性检查。技能属性是模型和旧参考创建器共同踩到的系统默认陷阱，原先“通过当前检查”的阶段性结论据此更新。详细证据、适用边界和变化过程见[修订记录](prep-character-benchmark-audit.md)。

**用户最新口径**：不强制追溯法师逐级获取历史，spells.acquisition仅作诊断，不参与失败判定。仍检查本题法术总数、最高环阶、职业列表、准备数、法术位和火球术活动。DeepSeek A1 JS因此通过当前自动配置检查；A1工具组仍因技能关联属性错误未通过。首轮输入保持原样，后续v3共用skill已同步放宽，禁止把两版输入混算。

**实现与验证**：新运行默认v3，300秒超时由CLI/config控制；旧隐式控制文件和共用skill归档，v2判定器保留用于复现原判。adapter记录验收版本和hash，audit按原报告选择版本，regrade对保存快照统一复核。修正后在COS另建v3六张参考卡，全部读回通过；6正例、1合法技能替代、23错误快照，复制ID/gear迁移/内容丢失、真实A1缺陷与来源表示、adapter版本选择等回归通过。旧参考卡与模型卡均未修。可复跑命令与归档方式在手册中。

**本轮判断与下一步**：新benchmark能揭示前轮轻量任务没覆盖的技能、资源、子职等缺口，同时自身误判也已被实际卡揭示并修订。单样本不足以证明查询工具稳定提速；两组尚有配置缺陷，不能以调用数下降代替正确性，也不能据此宣布Qwen无法完成任务。下一轮先针对具体缺陷打磨skill与查询表达，再用同供应商、同上限、交错重复样本验证；不扩张为通用车卡引擎。

自动通过只认证列明配置，不认证全部语言、起始装备组合、未知扩展来源、最终说明和实战自动化。HP按体型骰数学平均、保留原HP基线并追溯CON，CR不重评，熟练沿用系统默认。auto pack未修改；来源identifier规范化及旧宏适配需求只记录在[todo](todo.md)。


### 27.1 工具组失败轨迹审查

已对照首轮原始tool call、返回和卡片快照，见[失败案例分析](prep-character-tool-failure-analysis.md)。确认：DeepSeek A组三题共享技能字段写入/回读盲点；Qwen B1主动误解不叠加攻击次数为不授予额外攻击，B2/B3知道缺项仍以限制结束。查询工具目前不含选定子职与源Actor上下文，且原生成长UUID与导入skill优先auto pack之间仍需二次查找。旧轻量NPC措辞与完整成长要求存在张力，因果仍待实验。下一步假设为先统一完成标准与API回读，再补查询表达；本次仅审查记录，没有改工具或重跑。


## 28. 统一完成标准与实际导入来源查询（已实现，模型效果待测）

按用户要求，本轮先消除查、写两端的来源错位。实验foundry_build_query v2新增subclassName和classRole，读回实际职业/子职/种族及导入能力文档，返回实际UUID、机制预览、累计授予和待选项。原生成长数据仅作basis引用，写入UUID优先auto pack；缺项或无法唯一匹配时返回已读取的系统来源并标明原因，不让模型重新猜包。描述裁剪显式标记，保留所选条目的实际活动/消耗/效果摘要；不注入完整宏脚本。

共用实验skill已统一：要求职业等级时必须落实累计能力和正常字段/必选项；不自动应用Trait不是可接受的交付缺口。额外攻击保留为能力，只是不叠加多重攻击次数；追加怪物必须核对属性差量。NPC指南补技能value/ability/total、豁免和活动集合读取方法，删除轻量NPC与完整职业成长相冲突的措辞。既有生产包文件未修改，auto pack未修改。

验证：4项回归覆盖实际导入来源、子职、级别过滤、版本不符、歧义回退、职业条件消歧与源文档不变；本地COS对法师/战士/牧师/游荡者及子职做只读查询，所有返回UUID可读回、身份匹配，见[来源核验](prep-build-query-v2-source-check.json)。仍有无法唯一匹配的施法/额外攻击条目回退系统包；这不是宣称auto pack不存在。当前只证明来源查询正确可用，不证明LLM提速或正确率提升。

下一轮两臂共用修正skill，JS查询对比新工具；输入版本标记character-v4-source-query-defaults，验收器仍v3。旧工具与skill归档于character-benchmark/history，旧24次试跑不改分、不混入新输入成绩。


## 29. 成长关联优先：查询 v3（2026-09-09）

纠正§28的设计与验证结论：UUID可读取并不证明来源正确。此前以系统包成长为basis、再匹配auto pack，漏掉战士职业已明确关联的“额外攻击（战士）”。v3删除这条跨包匹配路线：入口仍按项目自动化包优先策略选择；其后只展开选定职业、子职、种族自身的成长配置，读取准确关联UUID，保留来源与等级。显式链接到系统包也原样保留。缺失链接或空成长配置报告unresolved，不静默补齐或宣称规则完整。子职与职业分开累计，不实现自然语言冲突裁决；特殊规则只影响其明确修改的部分，不能以子职整体覆盖职业。

同步实验skill两臂的来源策略，输入版本character-v5-linked-growth，验收器仍v3。旧查询v2与skill已归档。未修改auto pack、生产工具或角色，未跑新模型成绩。

验证：5项回归覆盖原样关联、同名干扰、系统链接保留、缺失链接/空配置、子职与等级过滤和源对象不变。本地COS只读查询五级战士/冠军勇士，额外攻击正确指向auto pack的ug1Y5YXiU65DLpsB，战斗风格6选1，技能8选2，属性提升2点；无回退和未解决链接。完整实际输出见[五级战士查询输出](prep-build-query-v3-fighter-output.json)。本次只验证读取链路，不认证导入后的全套实战自动化或LLM收益。


## 30. 查询 v4：候选资格与子职法术表（2026-09-09）

承接§29邪魔邪术师实查：原样展开关联仍会输出越级候选，且描述预览会截断法术表。本轮使用本地dnd5e原生prerequisites.level筛除目标等级不满足的候选；保留requirements及附加条件状态，不把等级满足等同于已满足所有条件。prerequisites.items或未解释文本条件标记conditional，缺少足够结构信息时保留review-required，不猜测玩家已选魔契或法术。当前按目标等级选卡，不验证逐级获取历史。未知条件保留给模型判断，不实现自然语言规则裁判。

从子职完整HTML中的法术表解析UUID（先解析后截断），读取真实Spell文档，使用CONFIG.DND5E的契约/完整施法等级表过滤环阶，输出spellAccess和spellTables。2014契约施法子职表标为expanded-list-candidates-not-granted；其他表仅标linked-spells-not-assumed-granted，不能推断自动习得。此版支持这些HTML表与UUID格式，未声称覆盖所有来源表示、半施法或多职业合并。候选描述最多220字符并标记截断，活动只返回类型预览，实际导入仍保留原文档完整机制；不把缩略输出当成自动化验证。

7项回归通过，涵盖关联保留、缺失关联、等级排除、文本/条目前置条件、截断之后的法术链接及环阶过滤。本地COS查询邪术师/战士/法师/牧师及子职均解析成功，无断链报告。五级邪魔邪术师53个祈唤降至35个（18个越级排除、16个附加条件待满足），返回6个一至三环扩展法术和2枚三环契约位。紧凑JSON从61715字符降至53967字符，仍偏重；没有以牺牲条件或冒充完整机制来压缩。完整输出见[邪术师v4](prep-build-query-v4-warlock-output.json)，来源核验见[v4记录](prep-build-query-v4-source-check.json)。

输入版本character-v6-eligible-growth；未跑新模型对照，不宣称提速。auto pack未改，文本前置条件缺少结构关联的问题仅记入todo。下一步应以新输出实测模型正确率和往返数，再决定候选摘要是否需要进一步压缩。


## 31. 全职业逐项审查启动（进行中）

用户目标：每个职业选一个子职查询，发现不合理即修复，每个milestone提交。Milestone 1为查询v4与邪术师修复，commit 27db5ed。本地自动化包枚举确认13职业，含奇械师；五级固定用例见test/fixtures/build-query-class-cases.json。13次初查全部完成，摘要见[初查记录](prep-build-query-class-sweep-initial.json)，该记录是修复前证据，不是全绿认证。

初查待处理：半施法的spellAccess为空导致圣武士/奇械师法术表越级；奇械师注法候选池为空；逸闻学院没有ItemGrant需检查其他成长类型；塑能学者未出现在所选子职授予中需查源描述；游侠必得宿敌/探索与替代选择可能重复；德鲁伊一项可选能力链接标记版本冲突。包数据禁止修改，确认属于源数据的问题记录todo，查询负责如实暴露。所有职业仍需逐项检查和修复后复核，目标尚未完成。


### 31.1 Milestone：半施法与动态类别选择

查询v5复用本地系统progression divisor/roundUp及单职业取整逻辑，修正五级圣武士、游侠、奇械师为二环上限；圣武士/炼金师表各保留4法术。保留2014一级半施法无槽的边界测试，奇械师一级可施法。空ItemChoice按系统配置的feat类型/子类型，在选定包内展开；仍无候选明确报告unresolved，不能当作完成。

10项单元回归通过，13职业实查完整输出与工具hash见[查询v5复查](prep-build-query-class-sweep-v5.json)。此里程碑只解决上述读取问题，不宣称全职业审查完成。奇械师候选类别在包内缺少标记，已确认授予的“奇械师注法”说明含真实UUID列表，下一里程碑继续处理这个描述关联。逸闻额外3技能由Trait正常返回；塑能学者在子职描述中正常保留，不能要求必有独立Item。


### 31.2 Milestone：描述关联与13职业审查收口

查询v6保留成长hint，展开授予文档内纯UUID列表；叙述示例和其他子职导航不当候选。实查确认奇械师14条注法链接全部断开，因此报告诊断并按用户纪律列todo，不用名称猜测覆盖源关系。其它职业来源/数值/候选逐项审查见[13职业审查表](prep-build-query-class-review.md)，最终证据见[完整v6输出](prep-build-query-class-sweep-v6.json)。

11项逻辑回归、14项当前指纹及实卡来源快照检查通过。本轮每职业一个子职的五级查询审查完成；工具能修的读取问题已修，必须改包的缺项/冲突已记录背景、需求和思路，auto pack保持不改。此结论不扩张为所有子职所有等级、完整车卡或LLM提速认证。既有benchmark/生产skill其他工作区改动未混入本轮提交。


### 31.3 Benchmark精确输入复核

按用户要求，不再以五级全职业巡检代替实际六题。以cases.cjs生成A1/A2/A3/B1/B2/B3对应职业、子职、等级及A组种族，在COS只读查询。完整证据见[六题精确输出](prep-build-query-benchmark-exact.json)，6项边界检查通过：三级生命牧师4/2槽、4领域法术且无摧毁不死；三级盗贼偷袭2d6、2专精且无直觉闪避；五级法师4/3/2槽，冠军勇士专用额外攻击；种族均解析，无断链。查询工具未发现新的大错误。

但确认当前v3验收器仍将evocation-savant强制按独立feat验收，无法接受自动化包子职描述中已存在的塑能学者。战斗风格/牧师额外熟练已有等价分支，此项没有。这是影响A1/B3的已确认验收缺陷，不应修改工具去捏造来源或额外导入来迎合。此次仅诊断和记录，未修改历史验收器或重评分；下一轮模型试跑前需用新验收版本补等价判定与负例测试。


## 32. 下一轮benchmark运行准备完成

验收v4包装冻结的v3，新增经过实际来源核对的塑能学者子职描述等价表示，要求准确子职来源、职业关联和完整关键规则。默认adapter切至v4；v2/v3历史行为不变。两个合法表示和5类反例通过；六张已审参考卡均通过v4；原v3反例、保留内容与HP规则回归通过。
...
### 33.2 A2完成：首对双方成功，尚无稳定提速证据

累计8/24试次审计完成，2次通过，无超时。A2 Qwen JS/工具175.765/166.353秒，均失败；工具组避免了JS组的越级能力、HP及步速错误，但属性与技能关联属性仍错。A2 DeepSeek JS/工具157.879/149.603秒，均完整通过、均35调用。单次5.2%的耗时差不作稳定收益声明。

DeepSeek工具组仅查一次职业，但在NPC schema及熟练、HP等派生逻辑上继续探索，并12次调用PowerShell读取本地系统代码。下一轮候选改进是经过验证的NPC API操作指南，而不是据此扩大职业查询接口；须结合后四题归因再决定。本轮24试次保持冻结输入和原始成绩，继续运行。

### 33.3 纯车卡A组完成，正确率收益尚未成立

12/24试次完成，均在300秒内返回。A组三题Qwen JS/工具均0/3通过；DeepSeek JS 2/3、工具1/3。A3 Qwen JS/工具272.345/170.033秒、25/12调用，均失败；DeepSeek JS223.235秒/46调用通过，工具146.542秒/22调用因步速0与固定AC漏敏捷而失败。工具减少往返不等于正确完成收益；当前只有A2 DeepSeek可比较双方成功耗时。

复核A3 Qwen工具组实际写入，确认子职classIdentifier误写成life-domain（应cleric），名称首匹配又选到残暴轻锤；前者连带两项判分，不算两种根因。共同native skill已讲明技能ability正确写法及熟练派生，模型仍漏做，因此下一轮须区分知识缺失与说明未落实，不能只堆更多说明。步速缺少专门字段条目可列候选补充，但本批冻结不改。详细证据沿用[复核旁注](prep-character-reviewed-v7-notes.md)。继续B组怪物追加职业等级，尚未作整体工具取舍结论。

### 33.4 B1 Qwen双方通过：首个120秒内成功工具样本

狼人追加五级冠军勇士：JS130.132秒/17调用、工具99.754秒/11调用，均通过原有内容保留与完整配置检查。工具组少30.378秒（23.3%）、6调用，且进入120秒。这是正面匹配样本，仍只有一次，不作为稳定速度承诺。它同时说明Qwen可以完成本类任务，不能用A组0/3概括整个模型能力。累计14/24完成，DeepSeek B1及B2/B3继续运行。

### 33.5 B1四试次全部通过

DeepSeek JS120.233秒/26调用、工具54.093秒/13调用均通过，工具少66.140秒（55.0%）。B1两个模型均出现成功条件下的耗时与调用减少，是当前最清晰的正面案例；与A组正确率未提升并存，不能用单一平均数掩盖任务差异。仍无重复样本，不宣称稳定提速。累计16/24完成、7次通过，无超时，继续地精盗贼与兽人法师。

### 33.6 B2 Qwen：HP计算约束未落实

JS139.764秒/17调用通过；工具137.912秒/18调用仅HP16/17失败。工具组脚本提前把d6均值3.5取整为3，违反共同skill“只对最终和取整”的明确说明。此项归执行计算错误，不归查询缺内容；也说明已有文档约束并不保证执行遵守。累计18/24完成，继续余下六次。

### 33.7 B2 DeepSeek双方通过，继续最后施法者增量题

JS157.506秒/29调用、工具105.582秒/23调用均通过。工具少51.924秒（33.0%）、6调用；与Qwen本题工具组HP失败同时保留，不挑选单模型成功结果代表整题。累计20/24完成、10次通过，无超时。最后B3兽人法师四次用于检验施法者增量场景，整批结论待其完成。

### 33.8 Reviewed v7整批完成与本轮结论

24/24全部完成，启动器正常退出。24个组合唯一且齐全，输入文件及逐报告配置指纹核对通过，均300秒/high thinking，无超时、供应商错误或未结算工具调用。完整逐项判分与audit哈希已保存在[证据文件](prep-character-reviewed-v7-evidence.json)，结论见[整批报告](prep-character-reviewed-v7-results.md)。

正式通过率：Qwen JS/工具2/6与2/6；DeepSeek5/6与4/6；合计7/12与6/12。工具未提升总正确率。双方成功的5对中工具4快1慢，B1两模型正面、B3 DeepSeek反面（JS112.033秒/26调用，工具189.822秒/40调用）。Qwen B3工具通过、JS漏2个法术，耗时及调用近似。120秒内成功JS1/12、工具3/12，每组合仅一次，不宣称稳定概率或速度收益。

本轮支持保留查询方向但不扩大工具面，下一轮优先验证已知API说明与写后有效值验收的组织方式，区分缺知识、未遵守说明和多余源码探索。已有HP取整/技能/子职关联约束未落实，不能仅堆知识或封死模型校验。A1可选规则与默认护甲歧义先修订合同，旧判分保持不动。复测需同时覆盖失败案例和B1已获收益案例，正确率优先，重复交错验证稳定性。下一轮改造与重跑尚未执行；本轮目标完成，auto pack保持不改。
