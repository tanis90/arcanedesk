...
**安全组：从"兜底提示"升级为"部署期并行引导"**。云厂商默认安全组几乎只开 22/3389 + ICMP——放行 30000 不是"用户可能忘"，而是**几乎必然要做的一步**。所以 skill 不等 V3 失败才提示，而是在**镜像下载的一两分钟窗口里并行引导用户去开**（不阻塞下载），路径按接入时①问的厂商精确给：

| 厂商 | 控制台路径 | 规则内容 |
|---|---|---|
| 阿里云 | 云服务器 ECS → 实例 → 详情页点"安全组"名 → 配置规则 → 入方向 → 手动添加 | 自定义 TCP，端口 `30000/30000`，源 `0.0.0.0/0` |
| 腾讯云 | 云服务器 CVM → 安全组 → 入站规则 → 添加规则 | TCP，端口 `30000`，来源 `0.0.0.0/0` |
| 华为云 | 弹性云服务器 ECS → 安全组 → 入方向规则 → 添加 | TCP / `30000` / `0.0.0.0/0` |
| 其他/自管 | 通用指引：找"安全组 / 防火墙 / 入站规则"，放行 TCP 30000、来源所有 | 同上 |

话术带一句为什么：来源放开到所有是因为玩家来自任何网络，安全边界在 FVTT 账号密码体系（本方案 §4 的既定决策）。**确定性诊断**：V2 通而 V3 不通 ≈ 必是安全组（docker 发布端口绕过主机防火墙，该变量基本可排除）——出现这个组合直接回到上表话术，不排查别的；用户说开好后重测 V3。
