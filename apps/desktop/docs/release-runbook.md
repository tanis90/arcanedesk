...
- Local full-flow E2E (packaged app, zero production touch):
  ...
  3. Compose `update/private-beta/latest.yml` (+ `latest-mac.yml`) for 0.4.91
     under the server root (`generated/e2e-feed/update/private-beta/`), with
     `files[].url` as `/files/<artifact>` and sha512 (base64) of the 0.4.91
     installer; copy installer bytes to `generated/e2e-feed/files/`.
