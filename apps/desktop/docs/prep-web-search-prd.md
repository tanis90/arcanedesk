...
│                                                              │
│  启用方式                                                     │
│  (•) 关闭            备团助手不注册 web_search 工具             │
│  ( ) Arcane Spark    复用已登录的 Spark Key，用量计入 Spark 账单 │
│  ( ) 自带搜索服务     使用你自己的搜索 API Key                  │
│        服务    [智谱 z-ai ▾]        ← P1: 智谱/Brave; P2: Kimi/OpenAI │
│        API Key  [••••••••••••••]  [覆盖]                       │
│        自定义端点（高级）  [ ]                                  │
│           端点  [https://…………………]  Key [••••••]              │
│                                                              │
│  ⓘ 检测到当前模型服务商为 Kimi Coding Plan ——                   │
│    可复用该 Key 直接搜索（调用 api.kimi.com，消耗订阅额度）。     │
│                              [ 用 Kimi Key 启用搜索 ]           │
│                                                              │
│  搜索结果会保存到本地会话记录中（标题、链接与摘要）。             │
└──────────────────────────────────────────────────────────────┘
```
...
| 模式 | 凭据 | 行为 | 阶段 |
| --- | --- | --- | --- |
| 关闭（默认） | — | 不注册 `web_search` | P1 |
| Arcane Spark | 复用 Spark key | 调 `POST /v1/search`，用量并入 Spark 统一计量与账单 | P1 |
...
任一模式从「关闭」切换到启用（含一键复用 Kimi/OpenAI）时弹出确认，明确接收方与数据范围：
...
│                                                │
│    接收方   智谱（api.z.ai）                     │
│    发送内容 仅搜索关键词；不含备团文档正文、       │
│             世界数据与任何密钥                   │
│    费用    约 ¥0.01–0.05 / 次，使用你的 Key       │
│                                                │
│  助手将被要求只发送完成检索所需的最少关键词。       │
│  搜索结果（标题/链接/摘要）会保存在本地会话记录中。 │
│                                                │
│               [ 取消 ]   [ 我已了解，启用 ]      │
└────────────────────────────────────────────────┘
```

- Spark 模式行文不同：接收方为 Arcane Spark（服务端再转发至上游），费用计入套餐。
- Kimi/OpenAI 复用模式需写明"消耗对应订阅/按次费用"。确认记录落盘，换后端或换接收方
  （custom endpoint 变更）需重新确认。
...
- 新增文案统一进 `shared/i18n/messages.js` 字典的 `ss.*` 命名空间（跟随 `sm.*` / `sv.*`
  ...
  为准。设置 tab、确认弹窗、工具卡片、用量行、错误态、引导文案全部走 `t()` / `data-i18n`，
  禁止硬编码字符串。
- 含变量的文案（"本轮已用 {count} 次搜索"）用现有 `{param}` 插值，不在代码里拼接句子。
- en-US 文案普遍长于中文：设置行、确认弹窗、卡片在 en-US 下必须完整显示，不截断、不溢出、
  不换行破坏布局。
...
- 费用展示按后端币种原样（智谱 ¥、Brave $、OpenAI $），不做汇率换算；确认弹窗文案随
  所选后端切换币种与单位。
...
6. 首次启用确认：三种模式（Spark/BYOK/复用）均弹出且文案含接收方与费用；确认前无网络请求。
7. i18n：新增文案 zh-CN / en-US 齐全（`ss.*`）；en-US 下设置行、确认弹窗、工具卡片完整显示
