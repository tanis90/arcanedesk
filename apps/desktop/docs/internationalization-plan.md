...
- LLM 上游改 **DeepSeek 海外站**（不再用 OpenRouter，见 D3 修订）；海外 LLM 启用时机
  暂缓、首发 BYOK（策略不变）；M5/M6 后端 **2026-09-11 已部署并端到端验收**
  （见 M5/M6 段部署日记录），启用 = 只发 key + 主仓库目录项。
...
- **M5 启用时机暂缓（策略不变）**：首发 BYOK（零后端），Spark-intl 在需要发试用 key
  验证付费意愿时再启用；后端 **2026-09-11 已部署并验收**，启用 = 只发 key +
  主仓库 provider-catalog / voice 默认项（见 M5 段部署日记录）。
...
**部署日记录（2026-09-11，全部验收通过，ops feat/intl-backend 提交至 `48a1ba0`，本地未 push）：**

- Worker 已上线：`llm.arcanedesk.app`（healthz 200；无 Bearer → 401 `unauthorized`；
  假 key → 401 `key_invalid`）；`DEEPSEEK_API_KEY` 已 wrangler secret 注入；D1 三表已建
- 端到端真机验收：非流式调用扣减**精确**（上游 68 token = 双闸各扣 68）；流式
  `stream_options.include_usage` 假设**成立**（尾帧带 usage，68→164 分毫不差）；
  suspend → 403 / resume → 200 / rotate → 旧 key 立即失效、剩余额度 9493 全结转、
  新 key 可用；审计流 cli:issue | worker:consume×N | cli:suspend/resume/rotate 完整
- CLI 传输层对真实 D1 验证通过（`wrangler d1 execute --json` 结构 + `meta.changes`
  均与假设一致）；冒烟数据（internal/smoke-test 全家）已清库
- **部署日教训两条**：① wrangler 部署 route **不会自动建 DNS 记录**（OAuth token 无
  dns:write），`api`/`llm` 两条 AAAA `100::` 占位记录是控制台手工补的——以后加 Worker
  路由都要记得这一步；② Windows 上 CLI 不能 execFile `npx.cmd`，传输层改为
  `node services/arcane-spark-edge/node_modules/wrangler/bin/wrangler.js`
- 主仓库侧（provider-catalog / voice 默认值）随启用时机再做；启用 = 发 key + 这两个目录项
...
### M6：遥测海外通道（ops 新服务，M）— 已部署并验收（2026-09-11）
...
**部署日记录（2026-09-11，全部验收通过）：**

- Worker 已上线：`api.arcanedesk.app`（healthz 200；无凭证 → 401 `unauthorized`；
  合法批次 → `accepted:true` 且 R2 实际落对象；含禁列 `prompt` → 400 `forbidden_key`
  带行号；DELETE 精确删 1 个对象、再删为 0）
- R2 lifecycle **已启用**：规则 `telemetry-raw-30d`（前缀 `telemetry/raw`，30 天删除）
- **CF Rate Limiting 已上线并实测触发**：规则 `telemetry-ingest-guard`
  （`api.arcanedesk.app` + `/v1/telemetry/` 前缀，每 IP 每 10 秒 10 请求，超出阻止 10 秒）
  ——连发 15 请求实测：前 10 个 401（到达 Worker），第 11 起 429（边缘拦截）。
  注意 Free 计划只给 **1 条规则 + 10 秒期间一档**，已用满；以后要加第二条需升计划
  （届时 llm.arcanedesk.app 的防刷也应补一条，目前靠 Worker 内 D1 预检兜底）
- 拉取脚本仍待实现：需求已写死在 ops `spec.md`（`tools/telemetry-pull/`，
  sync 命令 + DuckDB 入口 + 只读前缀限定凭证），另行安排
