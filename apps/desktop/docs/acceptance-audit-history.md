> 历史快照（2026-09-07 整合前），不作为当前实施要求或进度。唯一技术方案见 [architecture.md](architecture.md)，当前状态见 [acceptance-audit.md](acceptance-audit.md)。文中旧交互、未完成标记和 goal 状态均仅描述当时记录。

# 多会话规格验收审计

日期：2026-09-07。当前复核基线：`f3c177b`（M8o4）；最初审计基线为 `26776eb`。依据：[spec](spec.md)、[architecture](architecture.md)。

用户在本轮验收中修订了关闭规则：× 无条件收起托盘，托盘退出直接停止并收尾；旧三选一不再是验收要求。当前关闭自动回归见 [原生验收记录的修订](windows-native-acceptance.md)。原 goal 的推进已按用户要求停止，以下保留此前证据及待验证范围。

用户随后修订侧栏为“手动置顶 + 按项目聚合 + 行内转圈”，见 [侧栏设计](sidebar-navigation.md)。此方案尚未实现；旧活动区及计数横栏的 E4/E19 验收不作为新侧栏的通过证据。
...
| 要求 | 当前证据及判断 |
| --- | --- |
| 6.1-1 新建/打开/跨模式/收起/最小化不停止 | E1 覆盖前三类与隐藏；E19 覆盖侧栏收起布局，M8n1 实际 Windows 最小化/恢复通过 |
...
| §7-2 合并提示、不遮挡输入 | ActivityView 多项计数及独立活动条；E19 增补两项完成提示合并，四种尺寸均验证输入焦点、草稿及控件边界；提示位于输入区上方 |
...
| 8.3-2 关闭窗口的三项决策及重新打开 | E1 原生窗口/托盘对象、程序决策/回调；Windows 关闭/取消/停止退出由 M8n1 通过；后台重新打开的托盘实际点击仍待 G7。当前仅声明 Windows 实测 |
...
| §9-4、5 共享资源冲突与保守串行 | E9；E18 实际 Foundry 世界竞争/取消通过，不防外部程序写入 |
...
| 3 执行状态唯一、即时与确认区别 | E8；前端停止请求由 M8i/E3 覆盖 |
...
| 8 工具不虚假成功、停止不虚假完成 | E3/E8/E9；E13/E21/E22 已验证实际工具状态、停止及残留操作等待 |
| 9 已读与通知去重可恢复 | E4/E10 |
| 10 会话/任务/事件诊断、不额外敏感正文 | E12 及 [runtime scope tests](../test/direct-foundry-runtime.test.mjs)；界面同步诊断由 M8l/E3 覆盖 |
