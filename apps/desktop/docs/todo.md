...
补充实查：奇械师注法说明IrZuNJvar506SCUa列出的14条UUID全部无法fromUuid读取（包括带Item段的完整格式），因此之前“已有准确可用链接”的判断撤回。需求是包维护者修复目标文档/链接与类别配置；当前工具报告逐条断链，不做名称替换。完整证据在[全职业v6](prep-build-query-class-sweep-v6.json)的artificer结果。
