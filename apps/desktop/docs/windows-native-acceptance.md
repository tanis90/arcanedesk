...
用户明确要求：× 直接收起到托盘，托盘菜单“退出”才主动退出应用。旧三选一弹窗已删除，以下 M8n1 实测记录作为历史保留，不再代表当前关闭规则。

当前自动回归入口为 `node apps/desktop/test/smoke-tray-lifecycle.mjs`：生产 Electron/IPC/SDK 验证空闲和运行中关闭均调用真实 hide、无确认弹窗、实例不退出；托盘菜单回调直接开始退出，慢停止期间显示反馈，实际 SDK 停止后销毁托盘。系统鼠标点击不由此测试证明。
