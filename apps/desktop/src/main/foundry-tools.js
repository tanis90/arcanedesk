...
export function createFoundryTools(host) {
  ...
  return [
    ...
    actorWrite("foundry_actor_advance", "actorAdvance", exact({ actorUuid: ref(), readRef: ref(), classUuid: ref(), subclassUuid: Type.Optional(ref()), raceUuid: Type.Optional(ref()), targetLevel: Type.Integer({ minimum: 1, maximum: 20 }), choices: Type.Optional(exact({ skills: Type.Optional(Type.Array(ref(), { maxItems: 10, uniqueItems: true })), tools: Type.Optional(Type.Array(ref(), { maxItems: 10, uniqueItems: true })), cantrips: Type.Optional(Type.Array(ref(), { maxItems: 20, uniqueItems: true })), preparedSpells: Type.Optional(Type.Array(ref(), { maxItems: 50, uniqueItems: true })), hp: Type.Optional(Type.Union([Type.Literal("max"), Type.Literal("avg")])), abilityScore: Type.Optional(Type.Record(ref(), Type.Integer({ minimum: 1, maximum: 2 }))) })), additionalItems: Type.Optional(Type.Array(grant, { maxItems: 50 })) }),
      "Advance an existing dnd5e Character through the native AdvancementManager. Supply source UUIDs and only choices the native steps require; HP, class features, resources and derived values are calculated by dnd5e. Requires a current Actor readRef. Unsupported or missing choices are rejected before commit."),
    defineTool({
