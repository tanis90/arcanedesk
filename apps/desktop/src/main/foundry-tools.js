...
export function createFoundryTools(host) {
  ...
  return [
    ...
    actorWrite("foundry_actor_advance", "actorAdvance", exact({ actorUuid: ref(), readRef: ref(), classUuid: ref(), subclassUuid: Type.Optional(ref()), raceUuid: Type.Optional(ref()), targetLevel: Type.Integer({ minimum: 1, maximum: 20 }), choices: Type.Optional(exact({ skills: Type.Optional(Type.Array(ref(), { maxItems: 10, uniqueItems: true })), tools: Type.Optional(Type.Array(ref(), { maxItems: 10, uniqueItems: true })), cantrips: Type.Optional(Type.Array(ref(), { maxItems: 20, uniqueItems: true })), preparedSpells: Type.Optional(Type.Array(ref(), { maxItems: 50, uniqueItems: true })), feats: Type.Optional(Type.Array(ref(), { maxItems: 10, uniqueItems: true })), hp: Type.Optional(Type.Union([Type.Literal("max"), Type.Literal("avg")])), abilityScore: Type.Optional(Type.Record(ref(), Type.Integer({ minimum: 1, maximum: 2 }))), languages: Type.Optional(Type.Array(ref(), { maxItems: 10, uniqueItems: true })), expertise: Type.Optional(Type.Array(ref(), { maxItems: 10, uniqueItems: true })) })), additionalItems: Type.Optional(Type.Array(grant, { maxItems: 50 })) }),
      "Advance an existing dnd5e Character or NPC through the native AdvancementManager. Supply source UUIDs and only choices the native steps require; HP, class features, resources and derived values are calculated by dnd5e. Skill/tool/language choices use native trait keys from each step's pool (e.g. skills:arc, languages:standard:elvish); spell/feat choices use exact compendium UUIDs from that step's candidate pool. Selections no native step consumes are reported as warnings. Expertise slots (marked mode 'expertise' in the advancement plan) fill choices.expertise, not choices.skills: every pick must already be proficient — on the sheet or chosen in an earlier skills/tools slot of this same call — and invalid picks reject the whole advance before any write; a post-write safety net reports EXPERTISE_NOT_LANDED if dnd5e still drops one. Requires a current Actor readRef. Missing or invalid choices are rejected before any world write; additionalItems are validated the same way — expectedName must be copied verbatim from browse output or omitted. Fresh actors (level 0 before advancement) automatically end at full spell slots and, for characters, full HP — reported as slotFill/hpFill in the receipt; advances of experienced actors never touch current HP or slots. For NPC extensions HP uses the monster size die per native dnd5e rules, never the class die, and hpFill never fires. The receipt's verification block reports the actor end state — abilities before/after with racial and ASI decomposition, subclass, race, movement, languages, trait proficiencies (including expertise-level skills and tools), proficiency bonus, spellcasting (with per-level spell counts), AC, hit dice, initiative, class scale values, item resources, advancement-granted item identities with activity counts and preserved pre-existing item count — reconcile against it directly without follow-up reads. Prep-only."),
    defineTool({
