...
async function runCase(caseCfg) {
  ...
  try {
    ...
    // 2-3. plan 两次：先拿子职业池按名称解析，再带 subclassUuid 出最终计划（NPC 无种族，不传 raceUuid）
    ...
    const subUuid = (subReq?.candidates ?? []).find(u => (subReq.candidateNames?.[u] ?? "").includes(caseCfg.subclass));
    ...
    if (caseCfg.spells === "known" || caseCfg.spells === "book") additionalItems.push(...pickSpells(caseCfg, classUuid, plan.spellBudget ?? {}));
    ...
    // 6. advance 一次写入
    const bySlot = fillChoices(caseCfg, plan, caseCfg.primary, new Set(additionalItems.map(g => g.uuid)));
    ...
    const result = cli("actor-advance", { world, requestId: reqId(), actorUuid, readState: read.readState,
      ...
      ...(caseCfg.spells === "fullList" ? { fullSpellList: true } : {}) }, { timeoutMs: 180000 });
    ...
    else assertCase(caseCfg, plan, result, problems);
  } catch (error) {
...
async function main() {
  ...
  const cases = CASES.filter(c => FILTER.test(c.id) || FILTER.test(c.cls));
  console.log(`e2e advance matrix: ${cases.length} 案（world=${WORLD_ID} port=${PORT} extended=${EXTENDED}）`);
  const notes = [];
  for (const c of cases) {
    const note = await runCase(c);
    notes.push(note);
    console.log(`${note.ok ? "✅" : "❌"} ${note.id} ${c.cls}×${c.npc ? c.monsterName : c.race} ${(note.ms / 1000).toFixed(1)}s${note.ok ? "" : " — " + note.problems.join(" | ").slice(0, 300)}`);
  }
  const stamp = new Date().toISOString().replace(/[:.]/g, "-");
  const report = path.join(OUT_DIR, `advance-matrix-${stamp}.json`);
  fs.writeFileSync(report, JSON.stringify({ world, port: PORT, extended: EXTENDED, cases: notes }, null, 2));
